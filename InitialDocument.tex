...
A ``decision tree'' XXX reference ??? XXX is a special method to represent the partial training set in a complete form. Based on the bias ``good representations are short'', the heuristics contruct a ``short'' decision tree. A basic heuristics is captured by the ID3 algorithm XXX reference ??? XXX.
